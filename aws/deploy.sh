...
# Fully deploy homemind frontend and backend (monorepo) to AWS:
...
#       MONGO_INITDB_DATABASE (optional; defaults to â€œhomemindâ€)
#
...
MONGO_INITDB_DATABASE="${MONGO_INITDB_DATABASE:-homemind}"
...
PINECONE_INDEX="${PINECONE_INDEX:-homemind-index}"
...
ECR_BACKEND_REPO="homemind-backend-repo"
ECR_FRONTEND_REPO="homemind-frontend-repo"
ECS_CLUSTER_NAME="homemind-ecs-cluster"
ALB_NAME="homemind-alb"
SEC_GRP_NAME="homemind-ecs-sg"
DOCDB_SUBNET_GROUP="homemind-docdb-subnet-group"
DOCDB_CLUSTER_ID="homemind-docdb-cluster"
DOCDB_INSTANCE_ID="homemind-docdb-instance1"
DOCDB_SECURITY_GROUP="homemind-docdb-sg"
DOCDB_PORT=27017
TASK_EXEC_ROLE_NAME="homemind-ecsTaskExecutionRole"
...
if [[ -z "$DEFAULT_VPC_ID" ]]; then
  echo "âŒ No default VPC found in region $AWS_REGION"
  exit 1
fi
...
if [[ -z "$DEFAULT_SUBNET_IDS" ]]; then
  echo "âŒ No subnets found in VPC $DEFAULT_VPC_ID"
  exit 1
fi
...
# -----------------------------------------------------------------------------
echo "ðŸ”¨ Creating ECR repositories (if not existing)..."
...
pushd server >/dev/null
  docker build -t homemind-backend .
  docker tag homemind-backend:latest "$BACKEND_ECR_URI:latest"
...
pushd frontend >/dev/null
  docker build -t homemind-frontend .
  docker tag homemind-frontend:latest "$FRONTEND_ECR_URI:latest"
...
# -----------------------------------------------------------------------------
echo "ðŸ‘¤ Creating IAM role for ECS task execution (if not existing)..."
...
if ! aws ec2 describe-security-groups --filters "Name=group-name,Values=$SEC_GRP_NAME" "Name=vpc-id,Values=$DEFAULT_VPC_ID" --region "$AWS_REGION" | grep -q "$SEC_GRP_NAME"; then
  ECS_SG_ID="$(aws ec2 create-security-group \
    ...
    --description "Security group for homemind ECS tasks" \
...
# -----------------------------------------------------------------------------
echo "ðŸ—„ï¸  Creating DocumentDB subnet group (if not existing)..."
if ! aws docdb describe-db-subnet-groups --db-subnet-group-name "$DOCDB_SUBNET_GROUP" --region "$AWS_REGION" >/dev/null 2>&1; then
  aws docdb create-db-subnet-group \
    ...
    --db-subnet-group-description "Subnet group for homemind DocumentDB" \
    --subnet-ids $DEFAULT_SUBNET_IDS \
    --region "$AWS_REGION" >/dev/null
fi

echo "ðŸ”’ Creating DocumentDB security group..."
if ! aws ec2 describe-security-groups --filters "Name=group-name,Values=$DOCDB_SECURITY_GROUP" "Name=vpc-id,Values=$DEFAULT_VPC_ID" --region "$AWS_REGION" | grep -q "$DOCDB_SECURITY_GROUP"; then
  DOCDB_SG_ID="$(aws ec2 create-security-group \
    ...
    --description "Security group for homemind DocumentDB cluster" \
    --vpc-id "$DEFAULT_VPC_ID" \
    --region "$AWS_REGION" \
    --query "GroupId" --output text)"
...
echo "ðŸ—„ï¸  Creating DocumentDB cluster (may take ~10 min)..."
...
if [[ ! -f rds-combined-ca-bundle.pem ]]; then
  echo "ðŸ“¥ Downloading RDS CA bundle for DocumentDB..."
...
# -----------------------------------------------------------------------------
echo "ðŸ‘· Creating ECS cluster (if not existing)..."
...
# -----------------------------------------------------------------------------
echo "ðŸ“ƒ Registering ECS task definition for backend..."
cat <<EOF > backend-task-def.json
{
  "family": "homemind-backend-task",
  "networkMode": "awsvpc",
  ...
  "containerDefinitions": [
    {
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/homemind-backend",
...
echo "ðŸ“ƒ Registering ECS task definition for frontend..."
cat <<EOF > frontend-task-def.json
{
  "family": "homemind-frontend-task",
  "networkMode": "awsvpc",
  "requiresCompatibilities": ["FARGATE"],
  "cpu": "512",
  "memory": "1024",
  ...
  "containerDefinitions": [
    {
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/homemind-frontend",
          "awslogs-region": "${AWS_REGION}",
          "awslogs-stream-prefix": "ecs"
        }
      }
...
# -----------------------------------------------------------------------------
echo "ðŸ”§ Creating Application Load Balancer (ALB)..."
...
echo "ðŸ”§ Creating target group for backend..."
...
echo "ðŸ”§ Creating target group for frontend..."
...
echo "ðŸ”§ Setting up HTTP listener on port 80..."
...
echo "ðŸ”§ Configuring path-based routing ( /api/* â†’ backend )..."
...
# -----------------------------------------------------------------------------
echo "ðŸš¢ Deploying ECS service for backend..."
if ! aws ecs describe-services --cluster "$ECS_CLUSTER_NAME" --services "homemind-backend-service" --region "$AWS_REGION" | grep -q "homemind-backend-service"; then
  aws ecs create-service \
    --cluster "$ECS_CLUSTER_NAME" \
    --service-name "homemind-backend-service" \
    --task-definition "homemind-backend-task" \
    --desired-count 2 \
    --launch-type FARGATE \
    --network-configuration "awsvpcConfiguration={subnets=[$DEFAULT_SUBNET_IDS],securityGroups=[$ECS_SG_ID],assignPublicIp=ENABLED}" \
    --load-balancers "targetGroupArn=$TG_BACKEND_ARN,containerName=homemind-backend,containerPort=3001" \
    --region "$AWS_REGION" >/dev/null
else
  aws ecs update-service \
    --cluster "$ECS_CLUSTER_NAME" \
    --service "homemind-backend-service" \
    --force-new-deployment \
    --region "$AWS_REGION" >/dev/null
fi

echo "ðŸš¢ Deploying ECS service for frontend..."
...
if ! aws ecs describe-services --cluster "$ECS_CLUSTER_NAME" --services "homemind-frontend-service" --region "$AWS_REGION" | grep -q "homemind-frontend-service"; then
  aws ecs create-service \
    --cluster "$ECS_CLUSTER_NAME" \
    --service-name "homemind-frontend-service" \
    --task-definition "homemind-frontend-task" \
    --desired-count 2 \
    --launch-type FARGATE \
    --network-configuration "awsvpcConfiguration={subnets=[$DEFAULT_SUBNET_IDS],securityGroups=[$ECS_SG_ID],assignPublicIp=ENABLED}" \
    --load-balancers "targetGroupArn=$TG_FRONTEND_ARN,containerName=homemind-frontend,containerPort=3000" \
    --region "$AWS_REGION" >/dev/null
else
  aws ecs update-service \
    --cluster "$ECS_CLUSTER_NAME" \
    --service "homemind-frontend-service" \
    --force-new-deployment \
    --region "$AWS_REGION" >/dev/null
fi
