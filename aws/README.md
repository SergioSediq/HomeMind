...
AWS deployment options for homemind. This folder includes:
...
make deploy-ecs-service \
  ...
  EXECUTION_ROLE_ARN=arn:aws:iam::123:role/homemind-ecs-execution-role \
  CONTAINER_IMAGE=123.dkr.ecr.us-east-1.amazonaws.com/homemind-backend:latest \
  SUBNET_IDS=subnet-aaa,subnet-bbb \
  SECURITY_GROUP_IDS=sg-abc123 \
  TARGET_GROUP_ARN=arn:aws:elasticloadbalancing:...:targetgroup/homemind-tg/abcd \
  MONGO_SECRET_ARN=arn:aws:secretsmanager:...:secret:homemind/mongo \
  JWT_SECRET_ARN=arn:aws:secretsmanager:...:secret:homemind/jwt \
  GOOGLE_SECRET_ARN=arn:aws:secretsmanager:...:secret:homemind/google \
  PINECONE_SECRET_ARN=arn:aws:secretsmanager:...:secret:homemind/pinecone
```

Outputs (subnet IDs, target group ARNs, IAM role ARNs) can be retrieved via:

```bash
aws cloudformation describe-stacks --stack-name homemind-vpc
```
...
```bash
make delete-stack STACK=homemind-ecs-service
make delete-stack STACK=homemind-ecs-cluster
make delete-stack STACK=homemind-alb
make delete-stack STACK=homemind-iam
make delete-stack STACK=homemind-vpc
```
