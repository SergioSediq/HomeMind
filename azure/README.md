...
Azure deployment for homemind using Azure Container Apps, modular Bicep, and Key Vault-backed secrets. This stack provisions a managed runtime with secure networking and observability. For the multi-cloud comparison table see `DEPLOYMENTS.md`.
...
```bash
# from repo root
az group create -g homemind-rg -l eastus
./azure/deploy.sh \
...
|-----------|-------------|
| `environmentName` | Resource name prefix (default: `homemind`). |
