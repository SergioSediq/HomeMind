# homemind RPC APIs Documentation
...
homemind implements four core gRPC services, each handling specific domain responsibilities:
...
homemind's tRPC implementation follows a hierarchical router structure that mirrors the application's domain model:
...
|----------|---------|------|------|------|
| **Type Safety** | Compile-time checking | âŒ Manual | âœ… Automatic | âœ… Generated |
| | Runtime validation | âœ… Libraries | âœ… Zod | âœ… Protobuf |
| | Refactoring safety | âŒ | âœ… | âš ï¸ Regenerate |
| **Performance** | Payload size | Baseline | Same as REST | -30% |
| | Latency | Baseline | Same as REST | -25% |
| | Throughput | 1x | 1x | 1.5x |
| | Connection reuse | âš ï¸ HTTP/1.1 | âš ï¸ HTTP/1.1 | âœ… HTTP/2 |
| **Streaming** | Server push | âŒ SSE/WS | âš ï¸ Subscriptions | âœ… Native |
| | Bidirectional | âŒ | âŒ | âœ… |
| | Backpressure | âŒ | âŒ | âœ… |
...
| **Compatibility** | Browser support | âœ… Universal | âœ… Universal | âš ï¸ Proxy needed |
| | Language support | âœ… Any | âŒ TypeScript | âœ… 10+ langs |
| | Legacy systems | âœ… | âŒ | âš ï¸ |
| **Features** | Caching | âœ… HTTP | âœ… React Query | âŒ Custom |
| | File upload | âœ… Multipart | âš ï¸ Base64 | âœ… Streaming |
| | Auth standards | âœ… OAuth/JWT | âœ… JWT | âš ï¸ Custom |
...
homemind implements several patterns for protocol interoperability:
...
*This comprehensive documentation covers all aspects of homemind's RPC implementations. For specific implementation details and code examples, refer to the respective service repositories.*
