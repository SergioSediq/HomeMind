#!/usr/bin/env bash
# GitLab deploy helper that wraps existing homemind deployment scripts.
...
case "${STRATEGY}" in
  ...
  rolling)
    ...
    ${KUBECTL_BIN} rollout restart "deployment/homemind-${SERVICE_NAME}" -n "${NAMESPACE}"
    ${KUBECTL_BIN} rollout status "deployment/homemind-${SERVICE_NAME}" -n "${NAMESPACE}"
    ;;
  *)
