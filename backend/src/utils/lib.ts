import { runhomemindAgent } from "../services/geminiAgent.service";

/**
 * Run the homemind agent to determine if we need to fetch
 * property data or not.
 *
 * @param message - The userâ€™s message
 ...
 */
export async function runhomemindAgentCheck(
  message: string,
  userContext = "",
  expertWeights: Record<string, number> = {},
): Promise<{ finalText: string; expertViews: Record<string, string> }> {
  const response = await runhomemindAgent(
    message,
    userContext,
    expertWeights,
  );
...
/**
 * Detect if a user message is a simple greeting or smallâ€‘talk.
 * (Deprecated - not being used anymore in the current codebase.)
 *
 * @param message - the userâ€™s message text
