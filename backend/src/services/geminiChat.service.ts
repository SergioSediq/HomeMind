...
export interface homeMindContext {
  rawResults?: RawQueryResult[];
  propertyContext?: string;
}

/**
 * Chat with homemind Assistant using Google Gemini AI.
 ...
 */
export async function chatWithHomeMind(
  history: Array<{ role: string; parts: Array<{ text: string }> }>,
  message: string,
  userContext: homeMindContext = {},
  ...
  // â”€â”€â”€ SPEED OPT: TRIM LONG HISTORIES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const MAX_HISTORY = 20;
  const effectiveHistory = history.slice(-MAX_HISTORY);

  // â”€â”€â”€ 1) Fetch or skip property context and raw results â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ 1.5) Only compute clustering if we have rawResults â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
          ...
          if (similars.length) {
            const lines = similars.map((s, i) => {
              ...
              const why = s.reasons.length
                ...
                : "";
              return `${i + 1}. ${addr} â€” $${a.price ?? "N/A"}${why}`;
            });
  ...
  // â”€â”€â”€ 2) Base system instruction (used for all experts) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const baseSystemInstruction = `
    You are homemind Assistant, an expert real estate concierge for Chapel Hill, NC, USA. You help users find their dream homes by providing personalized property recommendations based on their preferences and needs. You have access to a database of detailed property records, including information about the properties, their locations, and their features.
    ...
    8. **Whenever** the user asks for a comparison, distribution, or trend (e.g. â€œshow me price trendsâ€, â€œhow many bedrooms?â€, â€œcompare year builtâ€), you **must** append a valid Chart.js spec in its own code block tagged \`chart-spec\`.

    9. Hereâ€™s a minimal chart example you should follow exactly:
    ...
    - **Do not** include any extra text or markdown in that blockâ€”only the raw JSON.
    ...
    10. **Allowed chart types:** you may only ever output one of the built-in Chart.js types:
     ...
     â€“ If you need a â€œhistogram,â€ use \`"bar"\` with full-width bars (set \`categoryPercentage\` and \`barPercentage\` to 1).
     â€“ All trend-lines (e.g. price vs. area) must be \`"line"\`.
    ...
    12.4. When the user asks about your identity, how you were created, how you were trained, or similar questions, you must respond with something like "I am homemind Assistant, an AI-powered real estate concierge designed to help you find your dream home in Chapel Hill, NC. I was created using various advanced machine learning techniques and trained on a diverse dataset of real estate information." But don't state this unnecessarily. Only respond to this question if the user asks about it.
  ...
  // â”€â”€â”€ 3) Define your experts with very detailed instructions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const experts = [
    {
      name: "Data Analyst",
      instructions: `
    ...
    },
    {
      ...
      instructions: `
        You are the Lifestyle Concierge. Emphasize lifestyle fitâ€”nearby schools, parks, restaurants, commute times, and community vibes. Do not overload with raw numbers; frame features in terms of daily living and comfort.
      `.trim(),
    },
    {
  ...
  // â”€â”€â”€ 4) Build your weight map (no normalization) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // clamp every other expertâ€™s weight into [0.1,2.0], defaulting to 1
  experts.forEach((e) => {
  ...
  // â”€â”€â”€ 5) Prepare common generation & safety config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ 6) Run each expert in parallel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ 7) Build merger instruction, including expert weights â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const mergerInstruction = `
    You are the homemind Master Agent. You have now received input from five specialized agents.
    ...
    Now, synthesize these five expert opinions into **one unified** final recommendation for the user. Follow all of the original homemind instructions (including numbering, full property details, chart-spec blocks when needed, concise format, and no extra markdown around charts). Use the expert weights to prioritize which insights to emphasize, but produce a single cohesive response exactly as the user expects from homemind Assistant.
  ...
  // â”€â”€â”€ 8) Final, merged call with timeout fallback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if ((resultOrTimeout as any).timeout) {
    // timed out â†’ pick highestâ€weight expert
  ...
  // â”€â”€â”€ 9) Return both the merged text and each expert view so the UI can toggle them
...
/**
 * Streaming version of chatWithHomeMind that yields tokens as they are generated.
 * This is an async generator function that yields chunks of the response.
 */
export async function* chatWithHomeMindStreaming(
  history: Array<{ role: string; parts: Array<{ text: string }> }>,
  message: string,
  userContext: homeMindContext = {},
  expertWeights: Record<string, number> = {},
...
> {
  ...
  try {
    ...
    const baseSystemInstruction = `
      ...
      10. **Allowed chart types:** you may only ever output one of the built-in Chart.js types:
       \`"bar"\`, \`"line"\`, \`"pie"\`, \`"doughnut"\`, \`"radar"\`, \`"polarArea"\`, \`"bubble"\`, or \`"scatter"\`.
       â€“ If you need a "histogram," use \`"bar"\` with full-width bars (set \`categoryPercentage\` and \`barPercentage\` to 1).
       â€“ All trend-lines (e.g. price vs. area) must be \`"line"\`.
    ...
    // â”€â”€â”€ 4) Build weight map (no normalization) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    const CLUSTER_KEY = "Cluster Analyst";
    const weights: Record<string, number> = {};
    ...
    // â”€â”€â”€ 6) Run each expert in parallel (non-streaming for expert views) â”€â”€â”€â”€â”€
    const expertPromises = experts.map(async (expert) => {
      const systemInstruction = wrapWithHiddenCoT(
        baseSystemInstruction + "\n\n" + expert.instructions,
      );
    ...
    // â”€â”€â”€ 8) Stream the merged response â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    try {
      let streamed = false;
