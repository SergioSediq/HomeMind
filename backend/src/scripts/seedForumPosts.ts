...
const DEFAULT_POST_AUTHOR: SeedUser = {
  email: "system@homemind.com",
  username: "homemind Team",
};

const seedPosts: SeedPost[] = [
  {
    title: "Welcome to the homemind Community!",
    content:
    ...
    comments: [
      {
        content:
          "Thrilled to have everyone here. If you are new, check the pinned market FAQsâ€”we update them every Monday.",
        author: {
          email: "insights@homemind.com",
  ...
  },
  {
    ...
    author: {
      email: "advisor@homemind.com",
      username: "Neighborhood Advisor",
    },
    comments: [
      {
        content:
          "We have a new dataset on school ratingsâ€”try filtering by districts with walkability scores above 70. That surfaced some hidden gems for my clients.",
        author: DEFAULT_POST_AUTHOR,
      },
      {
        content:
  ...
  },
  {
    ...
    category: "Advice",
    author: {
      email: "guide@homemind.com",
      username: "First-Time Buyer Guide",
    },
    comments: [
      {
        content:
          "Start with a realistic maintenance budgetâ€”unexpected repairs pop up fast. I set aside 1% of the purchase price annually.",
        author: {
          email: "designer@homemind.com",
  ...
  },
  {
    ...
    comments: [
      {
        content:
          "Energy-efficient windows have been a win for my sellersâ€”buyers like the utility savings and appraisers notice.",
        author: {
          email: "greenliving@homemind.com",
  ...
  },
  {
    ...
    comments: [
      {
        content:
          "Buyers respond to transparent utility bills. If you have historic usage data, bring it to showingsâ€”it builds trust.",
        author: DEFAULT_POST_AUTHOR,
      },
      {
        content:
...
async function ensureSeederUser(profile: SeedUser) {
  ...
  if (!user) {
    ...
    await user.save();
    logger.info(
      `ðŸ‘¤ Created forum seeder user ${profile.username} (${profile.email})`,
    );
  }
...
async function seedWithRetry(
  ...
  while (attempt < maxRetries) {
    try {
      logger.info(`ðŸŒ± Forum seeding attempt ${attempt + 1}/${maxRetries}...`);
      ...
      for (const postData of seedPosts) {
        ...
        if (existing) {
          logger.info(`â­ï¸ Post "${postData.title}" already exists, skipping.`);
          continue;
        }
        ...
        logger.info(`âœ… Created post: "${postData.title}"`);
        postDoc = post;
      ...
      logger.info(
        `ðŸŽ‰ Forum seeding complete. Posts created: ${createdPosts.length}. Comments created: ${totalCommentsCreated}.`,
      );
      return;
    } catch (err: any) {
      attempt++;
      ...
      logger.error(`âŒ Seeding attempt ${attempt} failed: ${err.message}`);

      if (attempt < maxRetries) {
        logger.info(
          `ðŸ”„ Retrying in ${delay}ms... (${maxRetries - attempt} attempts remaining)`,
        );
        await new Promise((resolve) => setTimeout(resolve, delay));
      } else {
        logger.error(
          `ðŸ’¥ Failed to seed forum posts after ${maxRetries} attempts. Give up.`,
        );
      }
    }
  }
...
export async function runForumSeeding() {
  ...
  logger.info("ðŸš€ Forum seeding initiated in background...");
}
