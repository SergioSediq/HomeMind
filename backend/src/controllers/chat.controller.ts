...
/**
 * Main chat endpoint â€“ handles both loggedâ€‘in and guest users.
...
export const chat = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (req.user) {
      ...
      // run MoE pipeline
      const { finalText, expertViews } = await chatWithHomeMind(
        historyForGemini,
        message,
        {},
...
function adjustWeightsInPlace(
  ...
  if (expert && expert !== CLUSTER && wts[expert] != null) {
    ...
    // 2b) Global thumb: pick two distinct nonâ€cluster experts
...
export const chatStreaming = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (req.user) {
      ...
      try {
        for await (const chunk of chatWithHomeMindStreaming(
          historyForGemini,
          message,
          {},
          conversation.expertWeights,
    ...
    try {
      ...
      if (timedOut) {
        const timeoutNote =
          "\n\n---\n\nâš ï¸ **Note:** This response was cut off due to cloud infrastructure timeout limits (60 seconds). Please try rephrasing or breaking down your request into smaller parts.";
        fullText += timeoutNote;
        // Stream the timeout note to the client
        sendEvent("token", { token: timeoutNote });
