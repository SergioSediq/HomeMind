def run() {
  pipeline {
    ...
    environment {
      ...
      BACKEND_IMAGE = "${REGISTRY}/homemind-app-backend"
      FRONTEND_IMAGE = "${REGISTRY}/homemind-app-frontend"
      ...
      K8S_NAMESPACE = "${env.K8S_NAMESPACE ?: 'homemind'}"
    ...
    stage('Lint & Format') {
      ...
      stage('Vercel Deploy') {
        steps {
          sh '''
            ...
            echo "Production: https://homemind.vercel.app (build ${GIT_COMMIT[0..6]})"
          '''
        }
      }
      ...
      stage('Pipeline Done') {
        steps {
          echo 'HomeMind CI/CD pipeline completed successfully.'
        }
      }
    }
  }
