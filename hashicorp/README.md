...
Deploy homemind on any Kubernetes cluster with HashiCorp Consul for service discovery/service mesh and Nomad for batch or scheduled workloads. This stack layers on top of a standard Kubernetes deployment and is ideal for teams already invested in HashiCorp tooling. For a cloud-by-cloud comparison see `DEPLOYMENTS.md`.
...
```
hashicorp/
â”œâ”€ deploy.sh                # Wrapper for terraform init/plan/apply
â””â”€ terraform/
   â”œâ”€ main.tf               # Installs Consul + Nomad via Helm
   â”œâ”€ providers.tf          # Kubernetes, Helm, optional Consul providers
   â”œâ”€ variables.tf          # kubeconfig / namespace settings
   â”œâ”€ outputs.tf            # Consul UI / Nomad UI / mesh endpoints
   â””â”€ values/
      â”œâ”€ consul-values.yaml # Connect + sidecar defaults
      â””â”€ nomad-values.yaml  # Nomad cluster tuning
```
...
./deploy.sh \
  ...
  --context homemind-prod \
  --namespace homemind \
  --mesh-namespace homemind-mesh \
  --do-apply
...
Nomad is deployed for scheduled/batch jobs (e.g., data ingest, nightly analytics). It does not replace Kubernetes for the web stack, but can complement it for workflows that fit Nomadâ€™s scheduler model.
