...
resource "kubernetes_namespace" "mesh" {
  metadata {
    ...
    labels = merge(local.homemind_labels, {
      "mesh.hashicorp.com/consul" = "true"
    })
  }
}

resource "kubernetes_namespace" "homemind" {
  metadata {
    name = var.homemind_namespace
    labels = local.homemind_labels
    annotations = {
...
resource "kubernetes_network_policy" "allow_consul_mesh" {
  ...
  spec {
    ...
    ingress {
      from {
        namespace_selector {
          match_labels = local.homemind_labels
        }
      }
    }
