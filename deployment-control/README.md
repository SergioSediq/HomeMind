...
Modern UI and API to run homemind blue/green, canary, rolling, and scaling operations without the CLI. Built with Nuxt 3, Vue 3, TypeScript, and Tailwind CSS.
...
- Namespace: `homemind` (override per request)
...
- `GET /api/cluster/summary?namespace=homemind` - deployment/service snapshot
...
curl -X POST http://localhost:4100/api/deploy/canary \
  ...
  -d '{
    "serviceName": "backend",
    "image": "ghcr.io/you/homemind-backend:sha-123",
    "namespace": "homemind",
