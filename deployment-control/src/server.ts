...
app.post("/api/deploy/rolling", (req: Request, res: Response) => {
  ...
  const command = `${kubectl} rollout restart deployment/homemind-${serviceName} -n ${namespace} && ${kubectl} rollout status deployment/homemind-${serviceName} -n ${namespace}`;
...
app.post("/api/ops/scale", (req: Request, res: Response) => {
  ...
  const deploymentName = variant
    ? `homemind-${serviceName}-${variant}`
    : `homemind-${serviceName}`;
...
app.get("/api/cluster/summary", async (req: Request, res: Response) => {
  const namespace = (req.query.namespace as string) || "homemind";
  try {
