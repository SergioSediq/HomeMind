#!/bin/bash
# Manual Chaos Engineering Tests for homemind
# Use when Chaos Mesh is not available

set -euo pipefail

NAMESPACE="${NAMESPACE:-homemind}"
COLOR_RED='\033[0;31m'
...
echo -e "${COLOR_BLUE}=== homemind Chaos Engineering Tests ===${NC}"
echo ""
...
test_pod_deletion() {
  ...
  POD=$(kubectl get pods -n $NAMESPACE -l app=homemind-backend -o jsonpath='{.items[0].metadata.name}')
  ...
  sleep 5
  kubectl wait --for=condition=ready pod -l app=homemind-backend -n $NAMESPACE --timeout=60s

  echo -e "${COLOR_GREEN}âœ“ Pod deletion test passed${NC}"
  echo ""
}
...
test_resource_stress() {
  ...
  echo -e "${COLOR_GREEN}âœ“ Resource stress test completed${NC}"
  echo ""
}
...
test_database_failure() {
  ...
  sleep 10
  kubectl logs -l app=homemind-backend -n $NAMESPACE --tail=20 | grep -i "error\|connection" || true

  echo "Restoring MongoDB..."
  kubectl scale deployment/mongodb --replicas=$ORIGINAL_REPLICAS -n $NAMESPACE 2>/dev/null || true

  echo -e "${COLOR_GREEN}âœ“ Database failure test completed${NC}"
  echo ""
}
...
test_network_partition() {
  ...
  echo -e "${COLOR_GREEN}âœ“ Network partition test completed${NC}"
  echo ""
}
...
test_rolling_restart() {
  ...
  kubectl rollout restart deployment/homemind-backend -n $NAMESPACE

  echo "Monitoring rollout..."
  kubectl rollout status deployment/homemind-backend -n $NAMESPACE --timeout=5m

  echo "Verifying all pods are ready..."
  kubectl get pods -l app=homemind-backend -n $NAMESPACE

  echo -e "${COLOR_GREEN}âœ“ Rolling restart test passed${NC}"
  echo ""
}
...
test_high_traffic() {
  ...
  SVC_IP=$(kubectl get svc homemind-backend -n $NAMESPACE -o jsonpath='{.spec.clusterIP}')
  ...
  echo -e "${COLOR_GREEN}âœ“ High traffic test completed${NC}"
  echo ""
}
