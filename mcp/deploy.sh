...
usage() {
  cat <<USAGE
Deploy the homemind MCP server.
...
case "$target" in
  aws)
    ...
    STACK=${MCP_AWS_STACK:-homemind-mcp}
    ENV=${MCP_AWS_ENV:-homemind}
    PARAMS=(
      EnvironmentName=$ENV
      ...
      ApiBaseUrl=${MCP_AWS_API_BASE_URL:-https://homemind-backend.vercel.app}
      FrontendBaseUrl=${MCP_AWS_FRONTEND_BASE_URL:-https://homemind.vercel.app}
  ...
  azure)
    ...
    az deployment group create \
      ...
      --parameters \
        ...
        apiBaseUrl="${MCP_AZURE_API_BASE_URL:-https://homemind-backend.vercel.app}" \
        frontendBaseUrl="${MCP_AZURE_FRONTEND_BASE_URL:-https://homemind.vercel.app}" \
  ...
  gcp)
    ...
    DEPLOY=${MCP_GCP_DEPLOYMENT:-homemind-mcp}
    ...
    gcloud deployment-manager deployments "$ACTION" "$DEPLOY" \
      ...
      --properties "region=${MCP_GCP_REGION},image=${MCP_GCP_IMAGE},serviceAccount=${MCP_GCP_SERVICE_ACCOUNT},apiBaseUrl=${MCP_GCP_API_BASE_URL:-https://homemind-backend.vercel.app},frontendBaseUrl=${MCP_GCP_FRONTEND_BASE_URL:-https://homemind.vercel.app},cacheTtlMs=${MCP_GCP_CACHE_TTL_MS:-30000},cacheMax=${MCP_GCP_CACHE_MAX:-200}"
    ;;
  compose)
