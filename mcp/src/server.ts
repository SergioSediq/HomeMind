...
/**
 * homemind MCP server: exposes properties, graph, analytics, finance,
 * map, auth, commute, util, and system tools over stdio.
 */
const server = new McpServer(
  { name: "homemind-mcp", version: "0.1.0" },
  { capabilities: { tools: {} } },
);
...
console.error(
  `homemind-mcp starting (API_BASE_URL=${config.apiBaseUrl})... waiting for client on stdio`,
);
server
  .connect(transport)
  .then(() => {
    // eslint-disable-next-line no-console
    console.error("homemind-mcp connected.");
  })
  .catch((err) => {
    // eslint-disable-next-line no-console
