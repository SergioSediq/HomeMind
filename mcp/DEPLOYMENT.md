...
This document describes how to build, containerise, and run the homemind MCP server in production. The MCP server exposes homemind tools over Model Context Protocol stdio and is designed to run alongside clients such as the Agentic AI CLI.
...
# Build image locally
docker build -t homemind-mcp:latest .

# Tag & push to registry
docker tag homemind-mcp:latest ghcr.io/your-org/homemind-mcp:latest
docker push ghcr.io/your-org/homemind-mcp:latest
```
...
|----------|-------------|---------|
| `API_BASE_URL` | homemind backend API base URL | `https://homemind-backend.vercel.app` |
| `FRONTEND_BASE_URL` | homemind frontend base URL (for map links) | `https://homemind.vercel.app` |
...
services:
  mcp:
    image: ghcr.io/your-org/homemind-mcp:latest
    environment:
...
containers:
  - name: agentic-ai
    image: ghcr.io/your-org/homemind-agentic:latest
  - name: mcp-server
    image: ghcr.io/your-org/homemind-mcp:latest
...
- Restrict network egress if the MCP server is only meant to reach the homemind API.
- Monitor usage via container logs; enable debug only during troubleshooting.

## Integration Checklist

- [ ] Build & push `ghcr.io/your-org/homemind-mcp:latest`
...
- **AWS ECS** â€“ Deploy via [`aws/ecs-service.yaml`](aws/ecs-service.yaml) (see [`aws/README.md`](aws/README.md)).
- **Azure Container Apps** â€“ Use [`azure/containerapp.bicep`](azure/containerapp.bicep) with guidance in [`azure/README.md`](azure/README.md).
- **GCP Cloud Run** â€“ Provision using [`gcp/cloudrun.yaml`](gcp/cloudrun.yaml) (refer to [`gcp/README.md`](gcp/README.md)).
