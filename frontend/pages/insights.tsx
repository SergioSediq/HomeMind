...
function BreakdownChart({
  ...
  }, [labels.join("|"), values.join("|"), colors.join("|")]); // eslint-disable-line react-hooks/exhaustive-deps -- chart data keys
...
function RateSensitivityChart({
  ...
  }, [loanAmt, termYears]); // eslint-disable-line react-hooks/exhaustive-deps -- rates/values derived
...
function DownPaymentImpactChart({
  ...
  }, [price, rate, termYears]); // eslint-disable-line react-hooks/exhaustive-deps -- pcts/values derived
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-auto bg-background text-foreground">
          ...
          <div className="max-w-none">
            {content.split(/\n{2,}/).map((block, blockIndex) => {
              ...
              const textLines = restLines.filter((line) => !/^[-â€¢]/.test(line));
                ...
                blockParts.push(
                  ...
                  >
                    {bulletLines.map((item, itemIndex) => {
                      const itemText = item.replace(/^[-â€¢]\s*/, "").trim();
                      return (
                        <li
...
export default function InsightsPage() {
  ...
  return (
    <>
      <Head>
        <title>Insights & Tools | homemind</title>
        <meta
          name="description"
...
**What you need:**
â€¢ From ZPID: The starting property's Zillow ID
â€¢ To ZPID: The destination property's Zillow ID

**How it works:**
The tool searches for connections through:
â€¢ Same ZIP code relationships
â€¢ Same neighborhood relationships
â€¢ Property similarity edges

**Tips:**
â€¢ Use the "Get ZPID" button if you don't know a property's ZPID
â€¢ The visual graph shows how properties are connected
â€¢ Shorter paths indicate stronger relationships`}
                      />
                    </CardTitle>
                    <CardDescription>
...
**What you need:**
â€¢ ZPID: The property's Zillow ID to find similar homes for
â€¢ Limit: Number of similar properties to return (1-20)

**Similarity factors:**
â€¢ Same neighborhood (green nodes)
â€¢ Same ZIP code (orange nodes)
â€¢ Vector similarity based on features (purple nodes)

**Understanding results:**
â€¢ Each result shows a similarity score
â€¢ Reasons explain why properties are considered similar
â€¢ The visual graph displays relationships with color coding

**Use cases:**
â€¢ Finding comparable properties for pricing
â€¢ Discovering alternative homes to consider
â€¢ Market analysis in specific areas`}
                      />
                    </CardTitle>
                    <CardDescription>
                                ...
                                <div className="flex justify-between text-sm">
                                  <div>
                                    ...
                                    <div className="text-muted-foreground">
                                      {prettyMoney(item.property.price)} â€¢{" "}
                                      {item.property.bedrooms ?? "?"} bd â€¢{" "}
                                      {item.property.bathrooms ?? "?"} ba â€¢{" "}
...
**What you need:**
â€¢ Neighborhood Name: Enter the name (e.g., "Meadowmont")
â€¢ Limit: Number of sample properties to display (5-100)

**Information provided:**
â€¢ Total property count in the neighborhood
â€¢ Average home price
â€¢ Average square footage
â€¢ Sample list of actual properties
â€¢ Interactive force-directed graph visualization

**Tips:**
â€¢ The tool tries different capitalizations automatically
â€¢ Drag nodes in the graph to explore relationships
â€¢ Larger nodes indicate higher-priced properties
â€¢ Use for quick neighborhood market overviews`}
                      />
                    </CardTitle>
                    <CardDescription>
                    ...
                    {hoodData ? (
                      <div className="space-y-2">
                        <div className="text-sm">
                          ...
                          {prettyMoney(hoodData.avgPrice)} â€¢{" "}
                            ...
                            hoodData.properties
                              ...
                              .map((p: any, i: number) => (
                                ...
                                >
                                  ...
                                  <div className="text-muted-foreground">
                                    {prettyMoney(p.price)} â€¢ {p.bedrooms ?? "?"}{" "}
                                    bd â€¢ {p.bathrooms ?? "?"} ba
                                  </div>
                                </li>
                              ))}
...
**Main inputs:**
â€¢ Home Price: Total purchase price
â€¢ Down Payment: Percentage down (0-50%)
â€¢ Interest Rate: Annual percentage rate (APR)
â€¢ Term: Loan duration in years
â€¢ Property Tax Rate: Annual tax as % of home value
â€¢ Insurance: Monthly homeowners insurance
â€¢ HOA: Monthly HOA fees if applicable

**Visualizations:**
â€¢ Monthly Breakdown: Pie chart showing payment components
â€¢ Rate Sensitivity: How payment changes with interest rates
â€¢ Down Payment Impact: Effect of down payment on monthly costs

**Additional tools:**
â€¢ Affordability Estimator: Calculate max home price based on income
â€¢ Quick calculators: Price per square foot and down payment amounts

**Tips:**
â€¢ Use sliders for quick adjustments
â€¢ Charts update automatically
â€¢ Consider all costs, not just principal & interest`}
                      />
                    </CardTitle>
                    <CardDescription>
...
function NeighborhoodForceGraph({
  ...
  }, [name, JSON.stringify((properties || []).slice(0, 60))]); // eslint-disable-line react-hooks/exhaustive-deps -- properties slice for chart
            ...
            <div className="flex-1">
              <div className="rounded-md border border-border/60 bg-muted/30 p-3">
                <div className="text-xs text-muted-foreground flex items-start gap-2">
                  ...
                  <div>
                    You can search with any single field or mix a few â€” no need
                    to fill everything.
                  </div>
                </div>
                ...
                >
                  <div className="text-sm leading-tight">
                    ...
                    <div className="text-xs text-muted-foreground mt-0.5">
                      ZPID: {r.zpid} â€¢ {r.bedrooms ?? "?"} bd â€¢{" "}
                      {r.bathrooms ?? "?"} ba â€¢ {r.livingArea ?? "?"} sqft
                      {r.price ? ` â€¢ ${prettyMoney(r.price)}` : ""}
                    </div>
                  </div>
                  <Button
                    size="sm"
