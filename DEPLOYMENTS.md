...
homemind ships with five production-grade deployment tracks so you can choose the cloud that best fits your stack:

1. **AWS** â€“ ECS on Fargate behind an ALB with CodePipeline / CodeBuild CI/CD.
2. **Azure** â€“ Azure Container Apps + Key Vault, driven by Bicep modules and Azure DevOps.
3. **Google Cloud** â€“ Cloud Run with Cloud Build, Artifact Registry, and VPC Access.
4. **Oracle Cloud (OCI)** â€“ Terraform-based VCN, compute, optional load balancer, OCIR images, and Docker Compose runtime.
5. **HashiCorp Platform** â€“ Terraform-orchestrated Kubernetes (EKS/AKS/GKE or self-managed) with Consul service mesh and Nomad batch runners.

In addition, the monorepo already supports **Vercel** deployments for the frontend (and optional backend edge functions) for teams that prefer Vercelâ€™s serverless workflow.
...
For more information on homemind DevOps, CI/CD pipelines, monitoring, and troubleshooting, see ðŸ“˜ [DEVOPS.md](DEVOPS.md).
...
Each target shares the same containers and environment variables â€“ only the infrastructure wrapper changes.

---

## Advanced Deployment Strategies

homemind now supports enterprise-grade deployment strategies for zero-downtime deployments and safe progressive delivery.
...
- âœ… Full environment testing before traffic switch
- âœ… Zero-downtime deployments
- âš ï¸ Requires 2x resources during transition
...
**Key Features:**
- âœ… Real production testing with minimal user impact
- âœ… Automated health checks and metrics validation
- âœ… Progressive rollout with manual approval gates
...
ðŸ“˜ **[DEVOPS.md](DEVOPS.md)** - Complete DevOps and deployment strategy guide
...
- **Web UI** â€“ Vue 3 + Nuxt 3 frontend with Pinia state management.
- **API Server** â€“ Express + TypeScript backend handling deployment requests and job tracking.
- **Features**:
...
cd aws
make deploy-vpc
dgn=$(aws cloudformation describe-stacks --stack-name homemind-vpc --query 'Stacks[0].Outputs')
make deploy-iam
make deploy-alb VpcId=<vpc-id> PublicSubnetIds=<subnet-a,subnet-b>
make deploy-ecs-cluster
make deploy-ecs-service \
  CLUSTER_NAME=homemind-ecs-cluster \
  EXECUTION_ROLE_ARN=<role-arn> \
  CONTAINER_IMAGE=<account>.dkr.ecr.us-east-1.amazonaws.com/homemind-backend:latest \
...
- `cloudbuild.yaml` pipeline: npm ci + tests â†’ docker build/push â†’ Cloud Run deploy using secrets.
...
./gcp/deploy.sh \
  --project <PROJECT_ID> \
  --region us-east1 \
  --service-account homemind-run@<PROJECT_ID>.iam.gserviceaccount.com
```
...
- **Container Image** â€“ See `agentic-ai/Dockerfile`; build with `docker build -f agentic-ai/Dockerfile .` and push to your registry.
- **Docker Compose** â€“ `agentic-ai/docker-compose.yaml` starts the orchestrator with all dependencies (LLM/Pinecone/Neo4j) supplied via `.env`.
- **Kubernetes** â€“ Apply `agentic-ai/k8s/` manifests (ConfigMap, Secret, Deployment) to run the CLI in a cluster with tty/stdin enabled.
- **Cloud providers** â€“ Fargate template (`agentic-ai/aws/ecs-service.yaml`), Azure Container Apps Bicep (`agentic-ai/azure/containerapp.bicep`), and Cloud Run config (`agentic-ai/gcp/cloudrun.yaml`).
...
- **Container Image** â€“ `mcp/Dockerfile` packages the MCP stdio server for sidecar usage; build/push to `ghcr.io/your-org/homemind-mcp`.
- **Docker Compose** â€“ `mcp/docker-compose.yaml` runs the server with caches tuned via env vars.
- **Kubernetes Sidecar** â€“ `mcp/k8s/sidecar-example.yaml` demonstrates pairing the MCP container with the Agentic AI deployment in the same pod.
- **Cloud providers** â€“ Fargate template (`mcp/aws/ecs-service.yaml`), Azure Container Apps Bicep (`mcp/azure/containerapp.bicep`), and Cloud Run config (`mcp/gcp/cloudrun.yaml`).
...
| `K8S_NAMESPACE` | `homemind` | Kubernetes namespace for deployments. |
...
pipeline {
  environment {
    ...
    K8S_NAMESPACE = 'homemind-staging'
  }
}
```
...
See ðŸ“˜ [DEVOPS.md](DEVOPS.md) for the full workflow breakdown and deployment stages.
...
- **Stages**: lint â†’ test â†’ build â†’ security (npm audit) â†’ deploy (manual by default)
- **Key vars**: `DEPLOY_STRATEGY`, `IMAGE_TAG`, `SERVICE_NAME`, `NAMESPACE`
- **Kube auth**: Prefer GitLabâ€™s Kubernetes agent or protected CI variables for `KUBECONFIG`

See ðŸ“˜ [DEVOPS.md](DEVOPS.md) for recommended defaults and variable details.
...
See ðŸ“˜ [DEVOPS.md](DEVOPS.md) for multi-cloud CI/CD guidance and deployment strategies.
...
terraform init
terraform plan -var='kubeconfig_path=~/.kube/config' -var='context=homemind'

# Azure DevOps pipeline run
az pipelines run --name homemind-container-apps --variables IMAGE_TAG=$(git rev-parse --short HEAD)
...
docker build -f agentic-ai/Dockerfile -t ghcr.io/your-org/homemind-agentic:latest .

# Build MCP server image
docker build -t ghcr.io/your-org/homemind-mcp:latest mcp/
