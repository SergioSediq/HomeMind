...
```bash
helm install homemind ./helm/homemind \
  --namespace homemind --create-namespace
```

## Upgrade

```bash
helm upgrade --install homemind ./helm/homemind \
  --namespace homemind \
...
By default the chart expects a pre-created secret named `homemind-secrets`.
