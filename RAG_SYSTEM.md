# homemind RAG System Architecture
...
homemind uses a **Hybrid RAG (Retrieval-Augmented Generation)** architecture that combines:

ðŸ” **Vector Database (Pinecone)** - Semantic similarity search using embeddings  
ðŸ•¸ï¸ **Knowledge Graph (Neo4j Aura)** - Relationship-based reasoning and explainability  
ðŸ¤– **Multi-Expert AI System** - Specialized agents for different analysis domains  
ðŸ“Š **K-Means Clustering** - Property grouping for pattern recognition  
...
- âœ… **Semantic Understanding** (Vector DB): "Find homes matching this description"
- âœ… **Structural Context** (Knowledge Graph - Graph RAG): "These homes are in the same neighborhood"
- âœ… **Explainability**: "Recommended because: same zip code, similar price, nearby schools"
- âœ… **Richer Context**: Combines text similarity with geographic/demographic relationships
...
### Stage 1: Initial Ingestion (MongoDB â†’ Pinecone)
...
### Stage 2: Graph Ingestion (Pinecone â†’ Neo4j)
...
**Indexes & Constraints:**
- âœ… Unique constraint on `Property.zpid`
- âœ… Index on `Zip.code` for fast lookups
- âœ… Index on `Neighborhood.name` for fast lookups

---

## RAG Query Flow

### Complete Pipeline (User Query â†’ AI Response)
...
**Query Process:**
1. User text â†’ Google text-embedding-004 â†’ 768-dimensional vector
...
```
score = 0.5 Ã— |price_diff| / base_price
      + 0.3 Ã— |area_diff| / base_area
      + 0.1 Ã— |bed_diff|
      + 0.1 Ã— |bath_diff|
```
...
|--------|-------|-----------------|
| ðŸ’¹ **Data Analyst** | Numbers & trends | "Average price in this cluster: $470K, range: $420K-$520K" |
| ðŸ˜ï¸ **Lifestyle Concierge** | Daily living | "Southern Village: top-rated schools (9/10), 2 parks within 0.5mi" |
| ðŸ’° **Financial Advisor** | Money matters | "At $465K with 20% down: $2,487/mo mortgage (estimated)" |
| ðŸ—ºï¸ **Neighborhood Expert** | Community | "Low crime area, walkability score: 78, new shopping center opening 2026" |
| ðŸ“Š **Cluster Analyst** | Pattern recognition | "This property is in Cluster 1 (mid-range family homes), priced 5% below cluster avg" |
...
- User feedback adjusts weights:
  - ðŸ‘ Thumbs up: No change (all experts contributed well)
  - ðŸ‘Ž Thumbs down: Randomly adjust 2 non-cluster experts Â±0.2
...
ðŸ’¡ *These 3 properties are in Cluster 1 (mid-range family homes) 
...
| **User Trust** | Black box ðŸ¤” | Transparent reasoning âœ… |
...
**Use Cases:**
- "Why are Property A and Property C related?" â†’ Path: Aâ†’Neighborhoodâ†’Bâ†’Zipâ†’C
- "Show me all homes within 2 relationship hops" â†’ Graph traversal
- "Are these homes in the same school district?" â†’ Future: Add school nodes
...
flowchart LR
    ...
    C --> E[Weighted Score:<br/>0.92 Ã— 0.6 + 0.33 Ã— 0.4<br/>= 0.684]
    D1 --> E
    D2 --> E
    D3 --> E
    D4 --> E
...
- Properties 1, 2, 3 â†’ Same neighborhood (Southern Village)
- Properties 1, 2, 3, 4, 5 â†’ Same zip (27514)

ðŸ“ˆ Cluster Analysis:
- Cluster 0 (Luxury): 8 properties, avg $620K, 4+ beds
- Cluster 1 (Mid-range): 23 properties, avg $470K, 3 beds â† Most results here
- Cluster 2 (Budget): 14 properties, avg $380K, 2-3 beds
- Cluster 3 (Historic): 5 properties, avg $520K, varied

ðŸŽ¯ Top 3 properties are in Cluster 1 AND Southern Village
```
...
```mermaid
graph TB
    CTX[Combined Context] --> E1[ðŸ’¹ Data Analyst]
    CTX --> E2[ðŸ˜ï¸ Lifestyle Concierge]
    CTX --> E3[ðŸ’° Financial Advisor]
    CTX --> E4[ðŸ—ºï¸ Neighborhood Expert]
    CTX --> E5[ðŸ“Š Cluster Analyst]
...
| Expert | Key Insights |
|--------|-------------|
| ðŸ’¹ **Data Analyst** | "Cluster 1 average: $470K, your budget fits perfectly. 3-bed median is 1825 sqft." |
| ðŸ˜ï¸ **Lifestyle** | "Southern Village has top-rated schools (9/10), 2 parks within 0.5mi, walkability score 78" |
| ðŸ’° **Financial** | "At $465K with 20% down: $2,487/mo mortgage + $465 property tax = ~$3,200 total" |
| ðŸ—ºï¸ **Neighborhood** | "Low crime, family-friendly, new shopping center opening 2026, 15min to UNC campus" |
| ðŸ“Š **Cluster** | "These are mid-range family homes. The first property is 5% below cluster averageâ€”good value!" |
...
### ðŸ  **Top 3 Homes Under $500K with Excellent Schools**

#### **1. 123 Main St, Southern Village, Chapel Hill, NC 27514** - **$465K**
   - **Details:** 3 beds | 2 baths | 1,800 sqft | Built 2015
   - **Why this home?**
     - âœ… Same neighborhood as 5 other similar properties
     - âœ… In mid-range cluster (5% below averageâ€”great value!)
     - âœ… Top-rated schools nearby (9/10 rating)
   - **Estimated Monthly:** $2,487 mortgage + $465 tax = ~$3,200 total
   - [View on Zillow â†’](https://www.zillow.com/homedetails/12345_zpid/)

#### **2. 456 Oak Ave, Southern Village, Chapel Hill, NC 27514** - **$475K**
   - **Details:** 3 beds | 2.5 baths | 1,900 sqft | Built 2017
   - **Why this home?**
     - âœ… Same neighborhood and zip as #1
     - âœ… Slightly larger with half-bath addition
     - âœ… Walking distance to community parks
   - **Estimated Monthly:** $2,548 total
   - [View on Zillow â†’](https://www.zillow.com/homedetails/67890_zpid/)

#### **3. 789 Elm St, Southern Village, Chapel Hill, NC 27514** - **$485K**
   - **Details:** 3 beds | 2 baths | 1,850 sqft | Built 2016
   - **Why this home?**
     - âœ… Same neighborhood cluster
     - âœ… Newer construction
     - âœ… Quiet cul-de-sac location
   - **Estimated Monthly:** $2,603 total
   - [View on Zillow â†’](https://www.zillow.com/homedetails/22222_zpid/)
...
ðŸ’¡ **Why These 3?** All three properties are in Cluster 1 (mid-range family homes) within the **same neighborhood** (Southern Village), offering:
- Consistent community vibes
- Top-rated schools in the same district
- Similar property values (stable investment)
- Walkable amenities

ðŸŽ“ **School Info:** Southern Village Elementary (9/10), McDougle Middle (8/10), East Chapel Hill High (9/10)
...
**Current Indexes:**
```
âœ… Neo4j: Property.zpid (unique constraint)
âœ… Neo4j: Zip.code (index)
âœ… Neo4j: Neighborhood.name (index)
âœ… Pinecone: HNSW vector index (automatic)
```
...
**Current Capacity:**
- âœ… Pinecone: Handles 1M+ vectors easily
- âš ï¸ Neo4j: Free tier limit = 50K nodes, 175K relationships
- âœ… Gemini API: 15 RPM (rate limited by API key tier)
...
```mermaid
flowchart LR
    A[User Gives Feedback<br/>ðŸ‘ or ðŸ‘Ž] --> B{Thumbs Down?}
...
PINECONE_INDEX=homemind-properties
...
**Required Services:**
- âœ… MongoDB (property data storage)
- âœ… Pinecone (vector database)
- âœ… Neo4j (knowledge graph) - *optional, system works without it*
- âœ… Google AI API key (embeddings + Gemini LLM)
...
```bash
# MongoDB
MONGO_URI=mongodb://localhost:27017/homemind
...
| Feature | Vector-Only RAG | Hybrid RAG (homemind) |
|---------|----------------|-------------------------|
| **Search Method** | Semantic similarity only | Semantic + structural relationships |
| **Explainability** | âŒ "These homes match" | âœ… "Match because: same neighborhood, same zip, similar price" |
| **Context Richness** | Single dimension (text similarity) | Multi-dimensional (text + graph + clusters) |
| **Neighborhood Awareness** | âŒ May scatter results across areas | âœ… Clusters by geographic/community groups |
...
**Use Hybrid RAG if:**
- âœ… Domain has natural relationships (real estate neighborhoods, product categories, social networks)
- âœ… Users need to understand *why* recommendations are made
- âœ… Context richness improves user experience
- âœ… You can maintain a knowledge graph (extra infra)

**Use Vector-Only RAG if:**
- âœ… Relationships are not important in your domain
- âœ… Pure semantic matching is sufficient
- âœ… You need simplest possible setup
- âœ… Latency is critical (<3s requirement)

---

## Key Takeaways

### What Makes homemind's RAG Unique?

```mermaid
mindmap
  root((homemind<br/>Hybrid RAG))
    Vector DB
      Semantic similarity
      Fast retrieval
      10K+ properties
...
|-----------|--------|---------|
| **ðŸ” Explainability** | Builds user trust | "Same neighborhood, same zip code" vs. "Property matches" |
| **ðŸ˜ï¸ Coherence** | Better UX | 3 homes in same area vs. scattered across city |
| **ðŸ“Š Rich Context** | Fewer follow-ups | Price trends + schools + payments in one response |
| **ðŸ¤– Multi-Expert** | Comprehensive | Financial + lifestyle + market analysis together |
| **ðŸ“ˆ Clustering** | Pattern insights | "You're looking at mid-range family homes" |

### Real-World Benefits

**For Users:**
- ðŸŽ¯ More relevant recommendations (vector + graph filtering)
- ðŸ’¡ Understand *why* homes are suggested (transparency)
- ðŸ˜ï¸ Discover neighborhood patterns (community vibes)
- ðŸ“Š Visual insights (charts auto-generated)
- ðŸ’¬ Natural conversation (multi-expert perspectives)

**For Business:**
- ðŸ“ˆ Higher engagement (richer responses keep users on site)
- ðŸ‘ Better feedback scores (explainability builds trust)
- ðŸ”„ Lower bounce rate (comprehensive answers reduce searches)
- ðŸ’° Better conversions (informed users make decisions faster)

---

## Frequently Asked Questions

### Q: Do I need Neo4j to use homemind?
...
- Neo4j Aura: $0-65 (Free tier â†’ Professional)
