...
1. **Create infra** (Deployment Manager):
   ```bash
   gcloud deployment-manager deployments create homemind-network --config deployment/networking.yaml
   gcloud deployment-manager deployments create homemind-service-account --config deployment/service-account.yaml
   gcloud deployment-manager deployments create homemind-cloudrun --config deployment/cloudrun.yaml
   gcloud deployment-manager deployments create homemind-bucket --config deployment/storage-bucket.yaml
   ```
...
3. **Run Cloud Build**:
   ...
   gcloud builds submit --config cloudbuild.yaml \
     --substitutions=_REGION=us-east1,_SERVICE_ACCOUNT=homemind-run@$GOOGLE_CLOUD_PROJECT.iam.gserviceaccount.com
   ```

Prefer a single command? Use the helper script:

```bash
./deploy.sh --project $GOOGLE_CLOUD_PROJECT --region us-east1 --service-account homemind-run@$GOOGLE_CLOUD_PROJECT.iam.gserviceaccount.com
```
