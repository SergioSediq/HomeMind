...
usage() {
  cat <<USAGE
Deploy homemind to Google Cloud (Cloud Run + supporting resources).
...
if [[ -z "$SERVICE_ACCOUNT" ]]; then
  SERVICE_ACCOUNT="homemind-run@${PROJECT}.iam.gserviceaccount.com"
fi
...
if [[ $SKIP_INFRA -eq 0 ]]; then
  ensure_deployment homemind-network "$ROOT_DIR/deployment/networking.yaml"
  ensure_deployment homemind-service-account "$ROOT_DIR/deployment/service-account.yaml"
  ensure_deployment homemind-cloudrun "$ROOT_DIR/deployment/cloudrun.yaml"
  ensure_deployment homemind-storage "$ROOT_DIR/deployment/storage-bucket.yaml"
fi
