...
> [!NOTE]
> Travis CI is supported for CI validation, but homemind uses **Jenkins** as the primary CI/CD platform for production deployments (blue/green + canary + multiâ€‘cloud). See `jenkins/README.md` and `DEVOPS.md` for production CI/CD.
...
- [Travis CI vs Jenkins (homemind)](#travis-ci-vs-jenkins-homemind)
...
## Travis CI vs Jenkins (homemind)

| Feature | Travis CI | Jenkins (homemind) |
|---------|-----------|----------------------|
| Build/Test/Lint | âœ… | âœ… |
| Security Scanning | âŒ (manual) | âœ… (multi-layer) |
| Deployments | âŒ | âœ… Blue/Green + Canary |
| Multi-cloud | âŒ | âœ… AWS/Azure/GCP/K8s |
| Rollback | âŒ | âœ… Automated |
| Production-ready | âš ï¸ Basic CI | âœ… Enterprise-grade |
