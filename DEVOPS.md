...
This guide provides comprehensive documentation for homemind's DevOps practices, deployment strategies, CI/CD pipelines, and operational procedures.
...
homemind employs enterprise-grade DevOps practices with multiple deployment strategies to ensure zero-downtime deployments, rapid rollbacks, and safe progressive delivery of new features.
...
- **Stages**: checkout â†’ lint/format â†’ tests â†’ image builds â†’ security scan â†’ perf checks â†’ deploy
...
- **Legacy pipeline**: `ci.yml` (deprecated; kept for backward compatibility â€” prefer `workflow.yml`).
...
- **Key variables**:
  ...
  - `NAMESPACE`: Kubernetes namespace (default `homemind`)
...
- **Kube auth**: Prefer GitLabâ€™s Kubernetes agent or protected CI variables for `KUBECONFIG`. No Dockerfile changes are required.
...
homemind supports three primary deployment strategies, each suited for different scenarios.
...
| **Instant Rollback** | âœ… Immediate | âš ï¸ Gradual | âš ï¸ Re-deploy |
| **Resource Usage** | 2x during switch | 1.1-1.5x | 1x |
| **Testing in Production** | Limited | âœ… Extensive | Moderate |
...
kubectl get service homemind-backend -n homemind \
  -o jsonpath='{.spec.selector.version}'

# Instant rollback to previous slot
kubectl patch service homemind-backend -n homemind \
  -p '{"spec":{"selector":{"version":"blue"}}}'

# Verify traffic switch
kubectl get endpoints homemind-backend -n homemind
```
...
graph TB
    ...
    subgraph K8s["Kubernetes Service"]
        Svc[Service Selector: app=homemind-backend]
    end
    ...
    subgraph Stage3["Stage 3: 100% Canary"]
        ...
        Canary3[Canary: 4 replicas â†’ Stable]
    end
...
| **Final** | 0 â†’ 2 (new) | 4 â†’ 0 | 100% | Stable | - |
...
| `K8S_NAMESPACE` | `homemind` | Kubernetes namespace |
...
pipeline {
  environment {
    ...
    GCP_DEPLOY_ARGS = '--project homemind-prod --region us-central1'
...
```bash
# Create namespace
kubectl create namespace homemind

# Create secrets
kubectl create secret generic homemind-secrets \
...
# Create configmap
kubectl create configmap homemind-shared-config \
...
kubectl apply -k kubernetes/base -n homemind

# Apply production overlay
kubectl apply -k kubernetes/overlays/prod -n homemind

# Check deployment status
kubectl get deployments -n homemind
kubectl get pods -n homemind
kubectl get services -n homemind

# View logs
kubectl logs -f deployment/homemind-backend -n homemind
kubectl logs -f deployment/homemind-backend -n homemind --previous
...
# Autoscaling (HPA)
kubectl autoscale deployment homemind-backend \
  --cpu-percent=70 \
  --min=2 \
  --max=10 \
  -n homemind
...
homemind deployments expose Prometheus metrics via annotations:
...
kubectl logs -l app=homemind-backend -n homemind --tail=100

# Follow logs
kubectl logs -f deployment/homemind-backend -n homemind

# Logs from specific deployment slot
kubectl logs -l app=homemind-backend,version=blue -n homemind
kubectl logs -l app=homemind-backend,version=canary -n homemind

# Export logs for analysis
kubectl logs deployment/homemind-backend -n homemind \
...
aws docdb restore-db-cluster-to-point-in-time \
  --source-db-cluster-identifier homemind-prod \
  --db-cluster-identifier homemind-prod-restored \
...
# 5. Verify
kubectl get pods -n homemind
curl https://api.homemind.com/health
```
...
kubectl get events -n homemind         # Check cluster events
```
...
kubectl get pods -l version=canary -n homemind

# View canary logs
kubectl logs -l version=canary -n homemind

# Check service endpoints
kubectl get endpoints homemind-backend -n homemind

# Manual rollback
kubectl scale deployment/homemind-backend-canary --replicas=0 -n homemind
...
kubectl get service homemind-backend -n homemind -o yaml | grep version

# Verify deployments are ready
kubectl get deployments -l app=homemind-backend -n homemind
...
kubectl logs deployment/homemind-backend -n homemind | grep -i "memory\|heap"
```
...
kubectl exec -it deployment/homemind-backend -n homemind -- /bin/sh

# Copy files from pod
kubectl cp homemind/pod-name:/path/to/file ./local-file
...
# Network debugging
kubectl run debug --image=nicolaka/netshoot -it --rm -n homemind
```
