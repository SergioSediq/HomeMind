...
case "$TARGET" in
  aws)
    ...
    STACK=${AGENTIC_AWS_STACK:-homemind-agentic-ai}
    ENV_NAME=${AGENTIC_AWS_ENV:-homemind}
    RUNTIME=${AGENTIC_AWS_RUNTIME:-langgraph}
    PINECONE_INDEX=${AGENTIC_AWS_PINECONE_INDEX:-homemind-index}
  ...
  azure)
    ...
    az deployment group create \
      ...
      --parameters \
        ...
        pineconeIndex="${AGENTIC_AZURE_PINECONE_INDEX:-homemind-index}" \
  ...
  gcp)
    ...
    DEPLOYMENT=${AGENTIC_GCP_DEPLOYMENT:-homemind-agentic-ai}
    ...
    gcloud deployment-manager deployments "$ACTION" "$DEPLOYMENT" \
      ...
      --properties "region=${AGENTIC_GCP_REGION},image=${AGENTIC_GCP_IMAGE},serviceAccount=${AGENTIC_GCP_SERVICE_ACCOUNT},runtime=${AGENTIC_GCP_RUNTIME:-langgraph},pineconeIndex=${AGENTIC_GCP_PINECONE_INDEX:-homemind-index},neo4jUri=${AGENTIC_GCP_NEO4J_URI:-},neo4jUser=${AGENTIC_GCP_NEO4J_USER:-}"
    ;;
  compose)
