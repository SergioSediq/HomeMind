# Agentic AI Pipeline for homemind

Welcome to the Agentic AI CLI for homemind, a standalone multi-agent orchestration tool designed to assist with real estate market research and analysis. This CLI leverages multiple specialized agents to break down complex goals into manageable tasks, utilizing the Model Context Protocol (MCP) tools for data retrieval and processing.

In addition to being the CLI, this agentic pipeline is also being used in our main homemind backend API and frontend UI! Feel free to use, adapt, and extend it as needed and use it in your own projects.
...
- Standalone multiâ€‘agent orchestration CLI purposeâ€‘built for homemind research.
...
- Programmatic `homemindLangGraphRuntime` class to inject custom context, instructions, or additional tools per thread.

**Programmatic usage**
```ts
import { homemindLangGraphRuntime } from './lang/graph.js';

const runtime = new homemindLangGraphRuntime({
...
The CLI integrates with the local MCP server to access homemind backend tools:
