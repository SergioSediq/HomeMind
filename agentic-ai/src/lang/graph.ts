...
You are homemind, a real-estate research and analysis agent.
Use tools to:
...
export interface homemindRuntimeConfig {
  systemPrompt?: string;
  tools?: LangTool[];
...
export function createhomemindAgentGraph(config: {
  systemPrompt?: string;
  tools?: LangTool[];
  checkpointer?: ReturnType<typeof getCheckpointer>;
  llm?: ChatModel;
...
export class homemindLangGraphRuntime {
  private readonly defaults: homemindRuntimeConfig;
  ...
  constructor(defaults: homemindRuntimeConfig = {}) {
  ...
  async run(options: LangGraphRunOptions): Promise<LangGraphRunResult> {
    ...
    return await withCostTracking(costTracker, async () => {
      ...
      try {
        ...
        const { app } = createhomemindAgentGraph({
          systemPrompt,
          tools,
          checkpointer,
          llm: this.llm,
...
export async function runhomemindAgent(input: RunInput) {
